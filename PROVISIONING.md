# How to provision a new environment for take_my_money
...
* Enter webroot path: `/home/deployer/apps/take_my_money/shared/public/`

The `.pem` files created by `certbot-auto` need to be linked so Nginx can see them:

```
ln -s /etc/letsencrypt/live/example.com/fullchain.pem /etc/ssl/take_my_money.crt
ln -s /etc/letsencrypt/live/example.com/privkey.pem /etc/ssl/take_my_money.key
```
...
```
/opt/certbot/certbot-auto renew --webroot-path /home/deployer/apps/take_my_money/shared/public/ --dry-run
```

Finally, add this to the root crontab to enable auto-renewal:

```
28 5,17 * * * /opt/certbot/certbot-auto renew --quiet --no-self-upgrade --webroot-path /home/deployer/apps/take_my_money/shared/public/
```
