...
sudo systemctl <start|stop|restart|status> take_my_money-puma.service
sudo systemctl <start|stop|restart|status> take_my_money-sidekiq.service
```

Note that the `deployer` user has permission to restart `take_my_money-puma.service` and `take_my_money-sidekiq.service`, but nothing else. All other `systemctl` commands require `root`.
...
```
/home/deployer/apps/take_my_money/shared/.env.production
```

After making changes to this file, be sure to restart the application:

```
sudo systemctl restart take_my_money-puma.service
sudo systemctl restart take_my_money-sidekiq.service
```
